...
export default class GenIIICalculation {
    constructor(props) {
        ...
        if (this.ballName === 'master-ball') {
            ...
            this.probability = calculateGenIIICaptureProbability(
                this.props.captureRate,
                this.props.ballSettings[1].settings['generation-iii'],
    ...
    getExplaination() {
        ...
        if (this.ballName !== 'master-ball') {
            return (<Box>
                ...
                {/* eslint-disable-next-line max-len */}
                <BlockMath math={String.raw`{\color{blue}a} \approx \frac{(3 \times {\color{gold}${this.pokemonHpMax}} - 2 \times ${this.pokemonHpCurrent.toFixed(2)}) \times ${this.props.captureRate} \times {\color{fuchsia}${this.ballModifier}}}{3 \times {\color{gold}${this.pokemonHpMax}}}\times {\color{green}${this.statusAilment}} \approx ${this.a.toFixed(2)}`} />
                {/* eslint-disable-next-line max-len */}
