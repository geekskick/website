...
import { calculateGenIICaptureProbability } from './../calculations/gen_ii';
...
export default class GenIICalculation {
    constructor(props) {
        ...
        if (this.ballName === 'master-ball') {
            ...
            this.probability = calculateGenIICaptureProbability(this.props.captureRate,
                this.props.ballSettings[1].settings['generation-ii'],
    ...
    getExplaination() {
        ...
        if (this.ballName !== 'master-ball') {
            return (<Box>
                ...
                {/* eslint-disable-next-line max-len */}
                <BlockMath math={String.raw`{\color{blue}a} \approx max\left( \left\lfloor \frac{(3 \times {\color{gold}${this.pokemonHpMax}} - 2 \times ${this.pokemonHpCurrent.toFixed(2)}) \times {\color{purple}${this.rateModified}}}{3 \times {\color{gold}${this.pokemonHpMax}}} \right\rfloor, 1 \right) + {\color{green}${this.statusAilment}} \approx ${this.a.toFixed(2)}`} />
                {/* eslint-disable-next-line max-len */}
                This {this.halfHalf ? 'did' : 'didn\'t'} invoke the double halving of <InlineMath math="3 \times {\color{gold}\text{HP}_\text{max}}" /> and < InlineMath math="2 \times \text{HP}_\text{current}" />.
                {/* eslint-disable-next-line max-len */}
