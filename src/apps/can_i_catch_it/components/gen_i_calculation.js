...
export default class GenICalculation {
    constructor(props) {
        ...
        if (this.ballName === 'master-ball') {
            ...
            this.probability = calculateGenICaptureProbability(this.props.captureRate,
                this.props.ballSettings[1].settings['generation-i'],
        ...
        if (this.ballName !== 'master-ball') {
            return (<Box>
                ...
                <BlockMath math={String.raw`{\color{forestgreen}\text{statusAilment}} =  \begin{cases}
                                                                            ...
                                                                            \end{cases}`} />
                According to <Link
                    href={'https://pokemon.neoseeker.com/wiki/Statistic#HP'}
                >Neoseeker</Link> the formula for calculating
                <InlineMath math={String.raw`{\color{gold}\text{HP}_\text{max}}`} /> is:
                { /* eslint-disable-next-line max-len */}
